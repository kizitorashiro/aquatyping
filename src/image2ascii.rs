...
pub fn string2ascii(message: &str, height: f32, ch: char, ch2nd: Option<(usize, char)>, font_file: Option<&str>) -> Result<Char2DArray, String> {
    let font = if let Some(file) = font_file {
        ...
        let font_data = include_bytes!("../font/wqy-microhei/WenQuanYiMicroHei.ttf");
    ...
    let font_file = font_file.unwrap_or(&String::from("../font/wqy-microhei/WenQuanYiMicroHei.ttf"));
    let font_data = std::fs::read(font_file).unwrap();
    Font::try_from_vec(font_data).unwrap()
    let font = Vec::from(include_bytes!("") as &[u8]);
    let font = Font::try_from_vec(font).unwrap();
    let scale = Scale {
        x: size,
        y: size,
